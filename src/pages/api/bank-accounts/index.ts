import { create, getAll } from '@/server/repositories/bankAccount.repository';
...
const bankAccountHandler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  switch (method) {
    case 'GET': {
      const bankAccounts = await getAll(userId);
      res.status(HttpStatusCode.Ok).json(bankAccounts);
      break;
    }
    case 'POST': {
